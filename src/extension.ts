...
function updateDecorations(editor: vscode.TextEditor) {
  ...
  while ((match = ANSI_COLOR_REGEX.exec(text)) !== null) {
    ...
    if (ANSI_COLOR_MAP[colorCode]) {
      console.log(
        'Mapping found for color code:',
        colorCode,
        '→',
        ANSI_COLOR_MAP[colorCode]
      );
...
function getAnsiCodeAtPosition(
  document: vscode.TextDocument,
  position: vscode.Position
): { code: string; range: vscode.Range } | null {
...
async function showColorPicker(
  ...
  ansiInfo: { code: string; range: vscode.Range }
) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Register document color provider for native color picker integration
  const colorProvider = vscode.languages.registerColorProvider('*', {
    ...
    provideColorPresentations(
      ...
      const match = originalText.match(/\\(?:033|x1b)\[(?:(\d+);)?(\d+)m/);
